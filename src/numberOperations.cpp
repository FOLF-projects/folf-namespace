...
bool folf::numberOperations::checkPrime(const unsigned long long* num)
{
    ...
    {
        if (*num % j == 0) // result of 0 means its not a prime = it can be devised through another number except 0 and himself
        {
            return false;
        }
    }
...
int folf::numberOperations::randomNum(int maxNum) {
	std::srand((unsigned)time(nullptr)); // generates a semi-random number based on the current time
